    ...
    if (e->message_ptr) {
        ...
        while (e->message_ptr) {
            ...
            if (message.level == IMPORTANT_ERROR) {
                ...
                while (!isprint(getch())); // user must type a character
                clear();
                draw(e);
            }
            else
...
void handle_command(Editor *e)
{
    ...
    if (strcmp(command[0], "w") == 0) {
        ...
        if (force || !b->changed) {
            endwin();
            exit(0);
        } else push_message_log(e, "Unsaved changes!", ERROR);
        ...
        if (strcmp(command[1], "info") == 0) level = INFO;
        if (strcmp(command[1], "warn") == 0) level = WARNING;
        if (strcmp(command[1], "error") == 0) level = ERROR;
        if (strcmp(command[1], "i_error") == 0) level = IMPORTANT_ERROR;
...
void handle_input(Editor *e, int ch)
{
    switch (ch) {
        ...
        if (e->mode == NORMAL) {
            ...
            else if (ch == 127 || ch == KEY_DC)         e->cx++, backspace_char(e);
